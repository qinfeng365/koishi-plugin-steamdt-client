    ...
    async def fetch_page_with_scroll(self):
        ...
        async with async_playwright() as p:
            try:
                ...
                # 滚动加载更多数据
                logger.info(f"开始滚动加载数据（最多 {self.max_scrolls} 次）...")
                ...
                for scroll_count in range(self.max_scrolls):
                    ...
                    logger.debug(f"滚动 {scroll_count + 1}/{self.max_scrolls} - 页面高度: {last_height} -> {new_height}")
